...
def reset(): # 重置各个天体
    test.x,test.y,test.dx,test.dy = 20, 0, 0, earth.getOrbitSpeed(r)
    test2.x,test2.y,test2.dx,test2.dy = 0, 20, -earth.getOrbitSpeed(r)*1.3, 0
    test3.x,test3.y,test3.dx,test3.dy = 0, -20, earth.getOrbitSpeed(r)*math.sqrt(2), 0
...
r = 20
print('"地球"半径:', r)
print('"地球"的第一宇宙速度:', earth.getOrbitSpeed(r))

test = Star(1,(20,0), (0, earth.getOrbitSpeed(r))) # 检验第一宇宙速度
test.color("red")
test.shapesize(0.4)

print('"地球"的第二宇宙速度:', earth.getOrbitSpeed(r)*math.sqrt(2)) # 第一宇宙速度的√2倍

test2 = Star(1, (0,20), (-earth.getOrbitSpeed(r)*1.3, 0)) # 速度介于第一、第二宇宙速度之间
test2.color("gray")
test2.shapesize(0.5)

test3 = Star(1,(0, -20), (earth.getOrbitSpeed(r)*math.sqrt(2), 0)) # 检验第二宇宙速度, 观察到test2的轨迹是抛物线
test3.color("purple")
test3.shapesize(0.4)
...
while True:
    ...
    try:
        ...
        # 绘制标签
        w.clear()
        w.goto(test.x,test.y)
        w.write("v=v1", font=(None,12))
        w.goto(test2.x,test2.y)
        w.write("v1<v<v2", font=(None,12))
        w.goto(test3.x,test3.y)
        w.write("v=v2", font=(None,12))
