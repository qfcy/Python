...
def reset(): # 重置各个天体
    sat.x,sat.y,sat.dx,sat.dy = 100, 0, 0, 1000
...
ttk.Button(btns,text="卫星加速",command=acc_satellite).grid(row=1,column=1,columnspan=2)
ttk.Button(btns,text="卫星减速",command=slow_satellite).grid(row=1,column=3,columnspan=2)
...
while True:
    ...
    try:
        ...
        w.clear()
        w.goto(scr.window_width()//2-230,
             scr.window_height()//2-60)
        w.write("""地球、卫星距离: %.3f
卫星速度: %.3f""" % (
    math.hypot(sat.x-earth.x,sat.y-earth.y), # math.hypot(x,y)相当于math.sqrt(x**2+y**2)
    math.hypot(sat.dx,sat.dy))
                , font=(None,12))
