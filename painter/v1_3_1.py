...
def _compare_ver(ver1,ver2):
    #ver1大于ver2时返回真,否则返回假
    fmt_str="{:0<3}"
    ver1=eval(fmt_str.format(ver1.replace('.','')))
    ver2=eval(fmt_str.format(ver2.replace('.','')))
    return ver1>ver2
...
class DictFile(dict):
    ...
    def save(self,writeall=False,error_callback=None):
        try:
            self.file=open(self.filename,'w',encoding="utf_8")
    ...
    def create_widgets(self):
        ...
        ttk.Button(backcolor,text="...",width=2,
                   command=lambda:self.backcolor.set(
                       askcolor()[1] or self.backcolor.get())).pack(side=tk.LEFT)
        ...
        ttk.Button(strokecolor,text="...",width=2,
                   command=lambda:self.strokecolor.set(
                       askcolor()[1] or self.strokecolor.get())).pack(side=tk.LEFT)
...
class Painter():
    ...
    def create_menu(self):
        ...
        self.menu.add_command(label="撤销",state=tk.DISABLED,
                              command=self.undo)
    ...
    def setdefault(self):
        ...
        if not "backcolor" in self.data:self.data["backcolor"]=self.BACKCOLOR
        if not "strokecolor" in self.data:self.data["strokecolor"]=self.STROKECOLOR
    ...
    def openfile(self,filename):
        ...
        self.menu.entryconfig(0,
                    state=(tk.NORMAL if self.data["data"] else tk.DISABLED))
        if _compare_ver(self.data.get("ver",_ver),_ver):
...
当前版本: {}\n文件版本: {}""".format(_ver,self.data["ver"]))
    ...
    def save(self,filename=None):
        ...
        try:
            self.data["width"]=int(self.cv.winfo_width())-int(self.cv["bd"])*2
            self.data["height"]=int(self.cv.winfo_height())-int(self.cv["bd"])*2
        except tk.TclError:pass
        self.data.filename=filename
        self.data.save(error_callback=onerror)
        self.file_changed=False
        self.master.title("%s - %s" %(self.TITLE,self.filename))
    ...
    def clear(self):
        ...
        if msgbox.askyesno("提示","是否清除?"):
            self._clearcanvas()
            self.data["data"]=[]
            self.data["backcolor"]=self.cv["bg"]=self.BACKCOLOR
            self.file_changed=True
        ...
        for window in PropertyWindow.instances:
            if window.master is self:
                if event.type==tk.EventType.FocusIn:
                    ...
                    window.focus_force()
                else:window.wm_attributes("-topmost",False)
    def quit(self):
        for window in PropertyWindow.instances:
            if window.master is self:
                window.destroy()
