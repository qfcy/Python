...
class Star(Turtle):
    ...
    def init(self):
        if self.has_orbit:
            self.pendown()
    def step(self):
        self._pos += self.gravSys.dt*self.v

        d = self._pos-self.sun._pos
        v = self.getOrbitSpeed()
        self.v = d.rotate(95)* (1/abs(d)) *v # 对Vec对象不能用除法
        l.append(abs(d))
        self.gravSys.dt=0.000001*abs(d)**1.5*self.gravSys.dt_s
    def update(self):
