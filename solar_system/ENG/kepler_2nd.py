...
def sleep(seconds):
    # 重写time.sleep方法，避免其用于过短的时间（如小于0.017s）延迟过长
...
scr=Screen()
scr.title("Python gravity simulation of Kepler's second law")
scr.bgcolor("black")
scr.tracer(0,0)
...
while True:
    ...
    try:
        ...
        update()
        # 锁定FPS
        sleep(max(1 / FPS - (time.perf_counter()-t),0))
        fps["text"]='FPS: %d' % int(1 / (time.perf_counter()-t))
        ...
        if count%20==0 and d_t!=0: # d_t!=0：未暂停
            ...
            w.goto(scr.window_width()//2-360,
                 scr.window_height()//2-100)
            # 如果行星扫过面积÷时间 是一个定值, 则验证通过
            w.write("Time:%.4f"%t0+"\nSquare:%.4f"%S + \
              "\nSquare ÷ Time = %.4f"%(S/t0) + \
                "\nTip: Accelerate/decelerate planets to\ncalculate k under different orbits.", font=(None,12))
            t0=0
            S=0
        d.goto(p.x,p.y)
        # 算出3条边
